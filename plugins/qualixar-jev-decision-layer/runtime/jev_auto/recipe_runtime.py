...
def _catalog() -> list[dict[str, Any]]:
    ...
    if data is not None:
        ...
        # There was a source-only fallback here that rebuilt the catalog from
        # `runtime/recipes/` via `src.adl.recipes.registry`. Neither exists:
        # the module has never been in this repository and recipe sources live
        # at the repository root, not inside the runtime, so they are not part
        # of what ships. Reaching it raised ModuleNotFoundError -- an untyped
        # crash from the one path whose entire job was to cope with a missing
        # catalog. A fallback that cannot fall back is worse than none,
        # because it reads like a safety net nobody tests.
        #
        # The packaged catalog is generated by tools/build_recipes.py and its
        # hash is pinned in RUNTIME_MANIFEST.json. If it is absent, the
        # installation is incomplete, and saying so is the honest answer.
